...
public class Exporter : MonoBehaviour
{
    ...
    public void RunExport(str_molecule input)
    {
        ...
        for (int i = 0; i < input.atomIndices.Count; i++) // 1 for now
        {
            ...
            for (int j = 0; j < directions.Count; j++)
            {
                ...
                if (doubles[j])
                {
                    cutter.position = v/2;
                    cutter.up = v;
                    cutter.localScale = new Vector3((bondWidth+0.5f)/scl, v.magnitude * 0.6f, (bondWidth+0.5f)/scl);

                    cutter2.position = cutter.position;
                    cutter2.up = v;
                    cutter2.localScale = new Vector3((bondWidth+0.5f)/scl, v.magnitude * 0.6f, (bondWidth+0.5f)/scl);
        ...
        for (int i = 0; i < input.bonds.Count; i++)
        {
            ...
            if (x >= xLimit)
            {
                largestX = bondWidth+inBetweenSpacing;
